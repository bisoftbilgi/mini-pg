...
public class MD5Digest {
	 ...
	 * added by yasar
	 * 
	 * Postgres stores tempDigest in the system databases. tempDigest=MD5(password +
	 * user)
	 * 
	 * Encodes user/password/salt information in the following way: MD5( tempDigest+
	 * salt).
	 *
	 * 
	 * @param tempDigest is the value of MD5(password + user) .
	 * @param salt       A four-salt sent by the server.
	 * @return A 35-byte array, comprising the string "md5" and an MD5 digest.
	 */
	public static byte[] encode(byte[] tempDigest, byte[] salt) {
		MessageDigest md;
		byte[] passDigest;
		byte[] hexDigest = new byte[35];
		...
		try {
			...
			if (tempDigest[0] == 'm') {
				md.update(tempDigest, 3, 32);
			} else {
				md.update(tempDigest, 0, 32);
			}
			md.update(salt);
			passDigest = md.digest();
	 ...
	 *
	 * 
	 * @param initialDigest calculates the value of MD5(password + user) .
	 * @return A 35-byte array, comprising the string "md5" and an MD5 digest.
	 */
	public static byte[] initialDigest(byte[] user, byte[] password) {
		MessageDigest md;
		byte[] tempDigest;
		byte[] hexDigest = new byte[35];
		...
		try {
			...
			bytesToHex(tempDigest, hexDigest, 3);
			hexDigest[0] = (byte) 'm';
			hexDigest[1] = (byte) 'd';
			hexDigest[2] = (byte) '5';

		} catch (NoSuchAlgorithmException e) {
			throw new IllegalStateException("Unable to encode user/pass with MD5", e);
		}
		return hexDigest;
	}
